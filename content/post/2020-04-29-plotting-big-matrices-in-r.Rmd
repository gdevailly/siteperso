...
I'm trying to generate a column-centered signal, becoming stronger an the rows, while keeping some randomness. I'll leave the code for reproducibility reasons, but 
...
At first one might think that all is well. We can see some signal, at the centre, stronger on top than on the bottom. But with 20,000 rows in the matrix, we should have obtain a much less noisy image.
Here we have 20,000 lines in our matrix, and a few hundred pixels in this png image file. 
So R must somehow decide how to summarize the values of several cells to draw a single pixel.
...
Some graphical devices (for example `pdf()` but not `png()`), will render the `image()` differently depending on its parameter `useRaster` (`FALSE` by default). Puting it to `TRUE` might help in some situations. According to [Wikipedia](https://en.wikipedia.org/wiki/Rasterisation),
rasterisation *is the task of taking an image described in a vector graphics format [...] and converting it into a raster image (a series of pixels)*. The rasterisation algorythm will try to resume several cells of the matrix in a single pixel. Let's try it:
...
The rasterisation try to summerise all the different values behind each pixel by taking their mean.
But we can do the same process ourselves, with two benefits: we get rid of the variability
between the different pdf reader softwares, and it will work even for non-vector devices such as png,
greatly limiting the final weight of the image.

Th idea is to reduce the matrix size **before** making the plot by applying a function (for example, `mean()`) that will
...
Much better! Paradoxically, we can better grasp the complexity of the data by summarising it before plotting, instead of letting R poorly handle the overploting issue!
...
A statistical test is perform for each SNP and each expressed gene, resulting in many p-values.
...
The `Heatmap()` function does have some parameters that suggest using *rasterization* in cases of big matrices,
